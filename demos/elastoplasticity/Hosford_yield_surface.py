...
# The tolerance of this plane stress correction will be measured by the out-of-plane stress residual |\sigma_{zz}/E|. We define a tolerance `tol=1e-6` and a maximum number of iterations. We then loop over each load step increment, compute the stress for $\varpesilon_{zz}=0$ and then do the plane stress correction until converging.
...
for i, t in enumerate(times):
    ...
    while np.any(res_plane > tol) and k < niter_max:
        print(f"   Plane stress correction iteration {k+1}")
...
# Finally, we plot the resulting distribution of stresses and compare against the analytical equation of the Hosford yield surface.
