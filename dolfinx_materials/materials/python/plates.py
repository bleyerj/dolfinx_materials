...
class NielsenPlate(PythonMaterial):
    ...
    def set_cvxpy_model(self):
        ...
        obj = 0.5*cp.quad_form(self.M-self.M_el, np.linalg.inv(self.elastic_model.C))
        yp = self.yield_parameters
        Mxx = self.M[0]
        Myy = self.M[1]
        Mxy = self.M[2]/np.sqrt(2)
        cons = [cp.quad_over_lin(Mxy, yp["mxp"]-Mxx) <= yp["myp"] - Myy,
    ...
    def constitutive_update(self, χ, state):
        ...
        self.prob.solve(requires_grad=True, solver=cp.SCS, verbose=False, eps_abs=1e-6, eps_rel=1e-6)
        M = self.M.value
